...
export const findManyNotes = async (reqId: string, userId: number) => {
  try {
    let result;
    if (process.env.SQP_INJECTION == "true") {
      result = await conn.query(
        `select note_id AS noteId, user_id AS userId, title, description, create_at AS createAt from notes where user_id = ${userId}`
      );
    } else {
      result = await conn.execute(
        `select note_id AS noteId, user_id AS userId, title, description, create_at AS createAt from notes where user_id = ?`,
        [userId]
      );
    }
...
export const findNoteById = async (
  ...
  try {
    ...
    if (process.env.SQL_INJECTION == "true") {
      ...
      result = await conn.execute(
        `select note_id AS noteId, user_id AS userId, title, description, create_at AS createAt from notes where user_id = ? and note_id = ?`,
        [userId, noteId]
      );
    }
...
export const updateNote = async (
  ...
  try {
    ...
    if (process.env.SQL_INJECTION == "true") {
      result = await conn.query(
        `update notes set title = '${title}', description = '${description}' where user_id = ${userId} and note_id = ${noteId}`
      );
    } else {
      result = await conn.query(
        `update notes set title = ?, description = ? where user_id = ? and note_id = ?`,
...
export const createNote = async (
  ...
  try {
    ...
    if (process.env.SQP_INJECTION == "true") {
      result = await conn.query(
        `insert into notes(user_id, title,description) value('${userId}', '${title}', '${description}')`
      );
    } else {
      result = await conn.query(
        `insert into notes(user_id, title,description) value(?,?,?)`,
